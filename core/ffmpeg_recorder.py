...
class FFmpegRecorder:
    def __init__(self, hwnd, output_file, audio_device_name, framerate, logger_func, on_critical_error_callback=None):
        self.hwnd = hwnd
        self.final_output_file = output_file 
        self.audio_device_name = audio_device_name 
        self.framerate = framerate if framerate > 0 else DEFAULT_FRAMERATE
        self.logger = logger_func
        self.on_critical_error_callback = on_critical_error_callback # Новый callback
    ...
    def _add_error_message(self, message, is_critical=False):
        if message:
            ...
            if is_critical and self.on_critical_error_callback:
                ...
                # Запускаем callback в главном потоке, если это возможно (если он из Tkinter)
                # Пока просто вызываем напрямую, предполагая, что вызывающая сторона это учтет.
                # В идеале, callback должен быть thread-safe или использовать master.after()
                try:
    ...
    def _initialize_grabber(self):
        if not (self.hwnd and win32gui.IsWindow(self.hwnd)):
            # Эта ошибка критична для старта, но _add_error_message ее не вызовет как is_critical
            # self.error_message будет установлен, и start() вернет False
    ...
    def start(self):
        ...
        try:
            ...
            self._add_error_message(f"Ошибка при запуске процессов ffmpeg: {e_start}", is_critical=True) # Критическая, т.к. запись не начнется
    ...
    def _video_feed_loop(self):
        ...
        loop_internal_error_msg_obj = {"msg": None} 
        ...
        try:
            ...
            while True: 
                ...
                # Обновляем счетчик кадров здесь, до проверки stop_event после grab_frame
                # Это гарантирует, что get_frames_written() вернет актуальное значение,
                # даже если цикл прервется сразу после этого.
                if frame_bgr is not None: # Только если кадр успешно захвачен
                    # Не увеличиваем frames_written_in_loop_local здесь, а только self.frames_written_count
                    # frames_written_in_loop_local будет увеличен после успешной ЗАПИСИ в пайп.
                    # Это нужно для более точного отображения в таймере GUI того, что УЖЕ отправлено.
                    # self.frames_written_count = frames_written_in_loop_local + 1 # Предварительное обновление
                    pass # Обновление будет после успешной записи
                ...
                try:
                    ...
                    self.frames_written_count = frames_written_in_loop_local # Обновляем после успешной записи
            ...
            self.frames_written_count = frames_written_in_loop_local # Финальное обновление

            if loop_internal_error_msg_obj["msg"]:
                # Передаем is_critical=True, чтобы вызвать callback
                self._add_error_message(loop_internal_error_msg_obj["msg"], is_critical=True) 
        ...
        # Не выводим здесь acumul_errors, т.к. они уже были залогированы через _add_error_message
    ...
    def _stop_ffmpeg_process(self, process, process_name, timeout_graceful=3, timeout_signal=7, timeout_terminate=3, is_audio=False):
        # ... (логика остается такой же, но сообщения об ошибках будут добавляться через self._add_error_message)
            ...
            def _stderr_reader_func_local_stop_unique(pipe, buffer_list, stop_event): 
                try:
                    while not stop_event.is_set():
                        ...
                        if len(buffer_list) > 250: buffer_list.pop(0) 
                except: pass 
                finally:
                    try: 
        ...
        if return_code is None: 
            ...
            try: 
                ...
                try: 
                    ...
                    return_code = process.poll() 
                    if not (is_audio and return_code == 1): 
                        self._add_error_message(f"{process_name} принудительно завершен.")
        ...
        if not process_final_stderr_content and pipe_to_read_stop and not pipe_to_read_stop.closed:
            try:
                remaining_bytes_direct = pipe_to_read_stop.read() 
        ...
        # Не возвращаем ничего, ошибки добавляются в self.accumulated_error_messages
    ...
    def _mux_files(self):
        ...
        if not os.path.exists(self.temp_video_file) or os.path.getsize(self.temp_video_file) == 0:
            ...
            self._add_error_message(err_msg, is_critical=True) # Критично для mux
    ...
    def stop(self):
        ...
        if self.ffmpeg_audio_process:
            ...
            self._stop_ffmpeg_process(self.ffmpeg_audio_process, "FFmpegAudio", timeout_graceful=2, timeout_signal=10, timeout_terminate=3, is_audio=True)
            self.ffmpeg_audio_process = None 
