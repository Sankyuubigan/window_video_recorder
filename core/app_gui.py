...
class ScreenRecorderApp:
    def __init__(self, master):
        ...
        try: 
            ...
            if getattr(sys, 'frozen', False) and hasattr(sys, '_MEIPASS'):
                base_path_frozen = sys._MEIPASS 
                icon_path_in_meipass = os.path.join(base_path_frozen, icon_name)
                if os.path.exists(icon_path_in_meipass):
                    final_icon_path = icon_path_in_meipass
                else:
                    executable_dir = os.path.dirname(sys.executable)
                    icon_path_near_executable = os.path.join(executable_dir, icon_name)
                    if os.path.exists(icon_path_near_executable):
                        final_icon_path = icon_path_near_executable
            else:
                # Путь из /core в корень проекта: ../app_icon.ico
                ...
                if os.path.exists(icon_path_source_relative_to_core):
                    final_icon_path = icon_path_source_relative_to_core
                else: # Фоллбэк: ищем в папке core (маловероятно, но для полноты)
                    icon_path_in_core_dir = os.path.join(current_script_dir, icon_name)
                    if os.path.exists(icon_path_in_core_dir):
                        final_icon_path = icon_path_in_core_dir

            if final_icon_path and os.path.exists(final_icon_path):
                master.iconbitmap(default=final_icon_path)
                self.log_message(f"[AppGUI] Иконка установлена из: {final_icon_path}")
            else:
                # Логируем все проверенные пути для отладки, если иконка не найдена
                paths_checked_log = []
                if getattr(sys, 'frozen', False) and hasattr(sys, '_MEIPASS'):
                    paths_checked_log.append(f"MEIPASS: {os.path.join(sys._MEIPASS, icon_name)}")
                    paths_checked_log.append(f"ExecutableDir: {os.path.join(os.path.dirname(sys.executable), icon_name)}")
                else:
                    paths_checked_log.append(f"SourceRelative (../): {os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), icon_name)}")
                    paths_checked_log.append(f"SourceRelative (./): {os.path.join(os.path.dirname(os.path.abspath(__file__)), icon_name)}")
        ...
        master.title("Video Recorder v0.24 (GDI + FFmpeg-Python)") 
        ...
        self.recording_timer = None # Будет инициализирован в _setup_gui
        self.settings = {} 
    ...
    def populate_audio_device_lists(self):
        ...
        def set_combo(combo, prev_val, keywords, exclude_vals=None):
            ...
            if val_to_set and val_to_set not in current_exclude_vals : combo.set(val_to_set)
                ...
                if not found_by_keyword and self.audio_devices: combo.current(0) # Убедимся, что self.audio_devices не пуст
                elif not self.audio_devices: combo.set('') # Если вообще нет устройств
    ...
    def _perform_recording_logic(self):
        # recorder_instance уже создан в start_recording_async
        ...
        if success_start:
            ...
            self.master.after(0, self.recording_timer.start) # Запускаем таймер GUI
    ...
    def _update_status_recording_in_progress(self):
        if self.is_recording: 
            ...
            # Дополнительно, если хотим обновлять статус каждые N секунд, можно запланировать здесь
            # self.master.after(1000, self._update_status_recording_in_progress) 
            # Но это будет конфликтовать с обновлением таймера, лучше обновлять статус только при старте/стопе.
    ...
    def _handle_recording_result(self, success, message):
        ...
        if not success: 
            ...
            if not self.is_recording: # Должно быть False, если stop отработал
        ...
        if not self.is_recording:
            ...
            self.recording_timer.stop() # Останавливаем таймер, чтобы он показал финальное время
            # self.recording_timer.reset() # Если нужно сбросить на 00:00:00
            if not success: 
                 self.recorder_instance = None
                 self.current_output_file = ""
    ...
    def start_recording_async(self):
        ...
        self.recorder_instance = FFmpegRecorder(
            ...
            audio_device_name=first_active_audio_device, 
            framerate=DEFAULT_FRAMERATE, 
            logger_func=self.log_message
        )
        # Устанавливаем источник для таймера
        self.recording_timer.set_source(
            ...
            target_fps=self.recorder_instance.framerate # Используем framerate из рекордера
        )
    ...
    def _update_gui_for_recording_state(self, is_starting_or_is_recording): 
        ...
        # Комбобоксы (их состояние readonly не меняется, но значения могут быть выбраны только до записи)
        ...
        if not is_starting_or_is_recording: 
            ...
            if "Запись" not in current_status_text and "Ошибка" not in current_status_text and "остановлена" not in current_status_text :
    ...
    def stop_recording(self):
        if not self.is_recording or not self.recorder_instance:
            ...
            self._update_gui_for_recording_state(False)
            self.recording_timer.reset() # Сбрасываем на 00:00:00
        ...
        if error_msg_stop: 
            ...
            self._handle_recording_result(True, "Запись успешно остановлена.") # Сообщение для handle_result
